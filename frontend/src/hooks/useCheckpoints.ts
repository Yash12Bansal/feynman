...
export function useCheckpoints({
  ...
  const prefetchedRef = useRef<Map<string, CheckpointQuestion | null>>(
    new Map(),
  );
  ...
  const triggerForTopic = useCallback(
    (topicId: string) => {
      ...
      void (async () => {
        ...
        if (cp === undefined)
          cp = await fetchCheckpoint(topicId).catch(() => null);
  ...
  const submit = useCallback(
    async (selectedOption: string) => {
      ...
      setOutcome(out ?? { correct: false, answer: "", answer_audio_url: null });
