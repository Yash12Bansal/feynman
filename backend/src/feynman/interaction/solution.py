...
_TOOL_DESCRIPTION = "Emit whether the solution needs a diagram and, if so, the DesignDiagramSpec."
...
async def get_or_build_solution(question_id: str) -> Solution | None:
    ...
    steps, needed, spec = await _call_diagram_llm(
        build_solution_user_prompt(q_text=q.q_text, answer=q.answer, options=q.options),
...
async def build_question_visual(q: Question) -> None:
    ...
    try:
        await cache_question_diagram(q.question_id, diagram_needed=needed, diagram_spec=spec)
    except Exception:
