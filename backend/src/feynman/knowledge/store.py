...
def make_session_id(student_id: str, chapter_id: str, local_date: str) -> str:
    ...
    """
    digest = hashlib.sha256(f"{student_id}|{chapter_id}|{local_date}".encode()).hexdigest()
    return digest[:32]
...
class StudentGraphStore:
    ...
    async def complete_session(self, *, session_id: str, ended_at: int) -> None:
        ...
        """
        await self._run(
            "MATCH (ses:StudySession {session_id: $session_id}) SET ses.ended_at = $ended_at",
    ...
    async def get_chapter_memory(self, *, student_id: str, chapter_id: str) -> MemoryCard:
    ...
    async def get_global_memory(self, *, student_id: str, limit: int | None = None) -> MemoryCard:
    ...
    def _card(
        ...
        *,
        ...
        attempts = [AttemptMemory(**a) for a in (row.get("attempts") or []) if a is not None]
