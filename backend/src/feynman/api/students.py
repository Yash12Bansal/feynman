...
async def start_study_session(student_id: str, body: StartSessionRequest) -> StartSessionResponse:
...
async def get_chapter_memory(student_id: str, chapter_id: str) -> MemoryCard:
    ...
    store = StudentGraphStore.connect()
    try:
        return await store.get_chapter_memory(student_id=student_id, chapter_id=chapter_id)
    finally:
        await store.close()
