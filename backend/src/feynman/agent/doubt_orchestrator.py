"""Doubt branch orchestrator — happy-path determinism for student doubts.
...
parent state on push, restore it on pop, and gate resolution on a checklist so
the LLM cannot resolve a doubt before actually addressing it.

Phase 2A landed snapshot + restore + checklist + tool-call auto-tick. Phase 2B
adds the watchdog (time bounds), force-resolve, voice-keyword auto-tick, and
the `@state_constrained` decorator.

Hooks called from `tools.py`:
- `on_push(branch, related_concept)` after `push_branch` + `push_board`
- `is_resolution_allowed(branch_id)` before `resolve_doubt`'s pop
- `on_pop(branch, publish_visual=..., say=...)` after `pop_branch` + `pop_board`
- `on_tool_invoked(tool_name, branch_id)` from `_publish_visual`
- `on_voice_emitted(transcript, branch_id)` — STUB in 2A, full wiring in 2B
"""
...
class ChecklistItem(BaseModel):
    ...
    Items auto-tick when a tool listed in `auto_satisfied_by` is invoked, or via
    the explicit `mark_doubt_step_complete(step_index)` tool when the agent
    knows it has addressed the requirement but no auto-trigger fired.
    """
...
class DoubtOrchestrator:
    ...
    Owns no asyncio state in 2A beyond the (unused) `_timeout_tasks` map, which
    Phase 2B fills with `_timeout_watchdog` tasks. All heavy work runs on the
    event loop driven by `tools.py`.
    """
    ...
    async def on_push(
        ...
        *,
        ...
        # Phase 2B will replace this stub with `_timeout_watchdog`.
    ...
    def on_voice_emitted(self, transcript: str, branch_id: UUID | None) -> None:
        """STUB in Phase 2A — voice-keyword auto-tick wired in 2B.

        Phase 2A relies on tool-call auto-tick + `mark_doubt_step_complete`
        for completeness. The hook signature exists so 2B can fill in the
        body without a contract change.
        """
        if branch_id is None or not transcript:
            return
        # TODO(phase 2B): keyword match against checklist items' descriptions
        # and explicit keyword list from the plan.
        logger.debug(
            "doubt_orchestrator.voice_hook_stub",
            branch_id=str(branch_id),
    ...
    async def _timeout_watchdog(self, branch_id: UUID) -> None:
        """STUB in 2A — Phase 2B implements 60s soft nudge + 120s force-resolve."""
        # Park forever; 2B replaces with sleep + nudge + force-resolve.
