...
async def advance_concept(ctx: RunContext) -> str:
    ...
    if next_concept is not None:
        ...
        # Ensure current concept has a plan, and pre-plan the next one.
        # Usually new_index is already planned (fired during previous concept).
        # But if teacher advanced fast or planning failed, handle it here.
        ...
        if tc.curriculum and tc.lesson_plan:
            ...
            async def _ensure_plans() -> None:
                for idx in (new_index, new_index + 1):
                    if idx < tc.lesson_plan.total_concepts and idx not in tc.concept_plans:
                        result = await plan_concept(
                            idx, tc.curriculum, tc.lesson_plan,
                ...
                # If current concept's plan just arrived, rebuild prompt
