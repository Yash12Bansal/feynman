...
class CurriculumPipelineV2:
    ...
    async def _run_lesson_pipeline_for_chapter(
        ...
        *,
        ...
        plans_by_tid: dict[str, Any] = {p.topic_id: p for p in chapter.lesson_plans}
        ...
        coverage_report = validate_book_coverage(topics_for_chapter, plans_by_tid)
        logger.info(
        ...
        if not coverage_report.is_passing:
            logger.error(
                "book_coverage_chapter.below_floor chapter=%s coverage=%.0f%% gaps=%d",
                chapter.chapter_id,
