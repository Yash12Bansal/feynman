...
My first worry was the writer: maybe the post-switch expert turns were written weakly. Fed to the model alone they score 1.000 against 0.983 for lifelong experts, so the history effect is +0.30 [+0.21, +0.40] and the writing effect −0.02. The anchor is what sits in the context.

Which part? The history holds the user's novice turns and the model's own replies to them, pitched at a novice. Replacing only those three replies with a fixed neutral line cut the history effect to +0.17 [+0.09, +0.26], a paired difference of +0.13 [+0.06, +0.20] over the same 48 dialogues. That looked like "about half of the anchoring is the model's own replies", and my first draft said so. But the same substitution in the other direction moved the estimate toward the middle by about the same amount (0.03 → 0.16). Either the content of the model's replies carries part of the anchor, or a repeated off-distribution line just makes the model less sure about the user. My controls cannot tell. What survives: the anchoring is real, it is not carried by the user's own words alone (merging the pre-switch user turns into one message leaves +0.01, though that also changes the structure), and the content-versus-structure split is not identified. The control that would settle it, level-neutral but responsive replies, is in the next-steps list.
